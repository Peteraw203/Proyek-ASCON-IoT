...
def demo_print(data):
    ...
    for text, val in data:
        print("{text}:{align} {val} ({length} bytes)".format(text=text, align=((maxlen - len(text)) * " "), val=val, length=len(val)))

def demo_aead_c(variant, p, k, n, a):
    assert variant in ["Ascon-128", "Ascon-128a", "Ascon-80pq"]
    ...
    # key   = "asconciphertest1".encode('utf-8') # zero_bytes(keysize)
    # nonce = "asconcipher1test".encode('utf-8')     # zero_bytes(16)
    
    # associateddata = b"ASCON"
    #plaintext = str(p).encode('utf-8')
    plaintext      = p

    ciphertext        = ascon_encrypt(k, n, a, plaintext,  variant)
...
def demo_aead_p(variant, c, k, n, a):
    assert variant in ["Ascon-128", "Ascon-128a", "Ascon-80pq"]
    keysize = 20 if variant == "Ascon-80pq" else 16
    #print("=== demo encryption using {variant} ===".format(variant=variant))
    ...
    receivedplaintext = ascon_decrypt(key, nonce, associateddata, c, variant)
    #print(receivedplaintext)
    #if receivedplaintext == None: print("verification failed!")
